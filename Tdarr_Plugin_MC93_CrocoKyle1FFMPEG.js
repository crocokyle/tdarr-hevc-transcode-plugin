...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  response.infoLog += `Downscaling bitrate by a factor of ${inputs.bitrate_scaledown_factor}x from ${currentBitrate} kbps to ${targetBitrate} kbps.\n`;
  if (inputs.bitrate_ceiling !== '') {
    if (targetBitrate > inputs.bitrate_ceiling) {
      response.infoLog += `Target bitrate ${targetBitrate} kbps exceeds ceiling. Limiting to ${inputs.bitrate_ceiling} kbps.\n`;
      targetBitrate = Number(inputs.bitrate_ceiling)
    }
  }
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (codec_type === 'video') {
      ...
      if (
          ...
          && currentBitrate <= Number(inputs.bitrate_ceiling)
      ) {
        response.processFile = false;
      ...
      response.infoLog += `- Uses HVEC or vp9: ${(codec_name === 'hevc' || codec_name === 'vp9')}\n`
      response.infoLog += `- Resolution is 1080p: ${(height === 1080 || width === 1920)}\n`
      response.infoLog += `- Bitrate is lower than ceiling: ${currentBitrate <= Number(inputs.bitrate_ceiling)}\n`
