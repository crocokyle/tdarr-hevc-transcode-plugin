...
const details = () => ({
  id: 'Tdarr_Plugin_MC94_CrocoKyle1FFMPEG-QS',
  Stage: 'Pre-processing',
  Name: 'CrocoKyles Transcode via CPU',
  Type: 'Video',
  Operation: 'Transcode',
  Description: `Files not in HEVC will be transcoded to HEVC using a CPU through ffmpeg.
                All files not in the target quality profile will either be upscaled or downscaled to fit this resolution.
                This is a rewrite of Migz Transcode Using Intel iGPU & FFMPEG.`,
  Version: '1.0.1',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  var codecFlags = "-c:v libx265"; // Default to high-efficiency HEVC (H.265)

  // The default and fallthrough is now libx265 (HEVC)
  if (file.video_codec_name === 'h263') {
      // Standard CPU encoder for H.263
      codecFlags = '-c:v libx263';
  } else if (file.video_codec_name === 'h264') {
      // Standard CPU encoder for H.264 (AVC) - highly recommended
      codecFlags = '-c:v libx264'; 
  } else if (file.video_codec_name === 'mjpeg') {
      // Standard CPU encoder for MJPEG
      codecFlags = '-c:v mjpeg'; 
  } else if (file.video_codec_name === 'mpeg1') {
      // Standard CPU encoder for MPEG-1
      codecFlags = '-c:v mpeg1video'; 
  } else if (file.video_codec_name === 'mpeg2') {
      // Standard CPU encoder for MPEG-2
      codecFlags = '-c:v mpeg2video'; 
  } else if (file.video_codec_name === 'mpeg4') {
      // Standard CPU encoder for MPEG-4 Part 2 (often 'DivX' or 'Xvid')
      codecFlags = '-c:v mpeg4'; 
  } else if (file.video_codec_name === 'vc1') {
      // VC1 is usually decoded on the CPU, but encoding support is limited/non-standard.
      // We will use libx264 as a safe, modern replacement for VC1 output.
      codecFlags = '-c:v libx264';
  } else if (file.video_codec_name === 'vp8') {
      // Standard CPU encoder for VP8 (WebM)
      codecFlags = '-c:v libvpx';
  }

  response.preset += `${genpts}, -map 0 ${codecFlags} `
